...
| #230 resource visibility panel                          | 85%      | Read-only panel shipped 2026-07-30 (PR #265); visual map shipped 2026-07-31 (#261/PR #268). Ordering editor built 2026-08-10 as #277/PR #282 (document-block mechanism — **un-gated from worker#257 item 2**, which stays parked). Remaining: per-resource server indicator + taxonomy grouping display items           |
| #277 per-mode resource prioritization                   | 90%      | **PR #282 built + dual-approved 2026-08-10** (codex clean / grok APPROVE after 3 rounds; 703/703 tests), CI-green, awaiting merge. Ordering persists as a generated block in the mode doc's Tool Guidance (zero worker change). Post-merge scope: Elsy staging pass + the issue's prompt-adherence accuracy check       |
