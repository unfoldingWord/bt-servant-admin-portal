...
**Last prod deploy**: a Deploy Production run went out **2026-07-08** (run `28967840957`) — main HEAD at the time was `b033468`, so prod is presumably **v1.10.3** (verify with Ian; tracker previously said 2026-06-24/v1.9.0). Staging is at `991ea68` — everything above plus **#209 requires_group toggle, #261 topology map, #249 admin trump + org-wide language visibility**.
...
| #254 Baruch Sonnet → Opus                               | 95%      | baruch PR #20 changes-requested by Ian 07-30 (needs eval evidence). Eval harness built 2026-07-31 (9 scenarios/32 checks, real prompt assembly, sonnet-4-6 vs opus-4-8); run blocked on API key (Ian's lane), then evidence → re-review. |
| #260 slug visibility + label sync (option 4)            | 100%     | Shipped 2026-07-21 (PR #262 = `b33e803`, same-day file→ship). Dropdown slug subtitle + header slug line + post-rename display-name prompt. Issue open pending Elsy's staging pass.                                                       |
