...
export function buildPriorityEntries(
  ...
  //
  // "Frozen" means frozen against INCIDENTAL drift — a display change leaking
  // into the prompt, which buys nothing and costs a surprise save. It is not a
  // promise that the emitted text can never change: #281 deliberately added the
  // disclosure paragraph to `generatePriorityBlock`, which does make an
  // existing block regenerate differently and does enable Apply on a plain
  // open. The difference is that it was a one-time, reviewed change with the
  // panel taught to explain the resulting state rather than leave the user to
  // infer it — see `isOfferedBlockRefresh` and `priorityLengthVerdict`. A
  // change to THESE bytes still buys nothing, so the freeze stands here.
  //
